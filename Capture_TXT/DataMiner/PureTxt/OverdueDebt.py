...
class OverdueDebt(TextInterpreter):
    def create_df(self, text):
        ...
        return 'DIVIDA VENCIDA', df.reset_index(drop = True)

    def _capture_information(self, text):
        aux_text = re.split('  +',text)

        return {'DATA':aux_text[0].split(' ')[0],\
                'MODALIDADE':aux_text[0].split(' ')[1], \
                'VALOR': aux_text[2].split(' ')[0],
                'TITULO':aux_text[2].split(' ')[1],
                'INST_COBRADORA':aux_text[3],
                'LOCAL':np.nan}


    def create_df_by_text(self, text):
        aux_text = text
        index_base = aux_text.find('R451DIVIDA VENCIDA')
